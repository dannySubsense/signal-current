...
7. A strategy is not validated by its own author's rerun, however deterministic the replay. Promotion to StrategyArtifact requires that someone other than the original author (or authoring agent) independently reruns the validation against the same content-addressed inputs and obtains the same result. This requirement is additional to, not a substitute for, the deterministic-replay requirement (§7 below) and the multiple-testing/selection-bias diagnostics requirement (§5 item 4 below) — those two constrain *the method* of validation; this one constrains *who checks it*. *(Reconciliation Matrix row 59; Reconciliation §6 item 14; traces directly to `docs/NORTHSTAR.md`'s Thesis: "A strategy only counts as validated if someone other than its author can rerun the same test and get the same result.")*
...
2. **Seedable stochastic computation.** Non-deterministic but seedable algorithms (e.g. randomized search, bootstrap resampling) must record seeds and environment, but a single run is never sufficient trust. They additionally require an immutable input snapshot plus replicated or statistical evidence across runs before promotion. *(Matrix row 22, read together with Matrix row 1's requirement that results be attributable to versioned inputs and environment.)* The specific requirement that a single seeded run is insufficient and that replicated or statistical evidence must be produced before promotion is not itself stated in Matrix row 22, which only requires that seeds and environment be recorded — this stronger requirement is an explicit synthesis, generalizing Matrix row 33's principle that "raw Sharpe is never sufficient evidence" after a search from the multiple-testing context to seedable stochastic computation generally. This is flagged as synthesis rather than presented as a one-to-one citation, following the same convention §10 below uses.
3. **Inherently non-replayable output.** Live LLM inference and live market-data feeds cannot be made replayable by recording a seed or environment metadata — there is no fixed input tuple to replay against. Such output MUST be classified as proposal material only and MUST NOT be certified as numerical evidence, regardless of seed-recording. *(Matrix row 18, extended to the live-data case by Matrix row 1's sole-numerical-authority principle.)* Matrix row 18 itself addresses only LLM-generated signals; it does not separately discuss live market-data feeds. Extending its treatment to live-data feeds is an explicit synthesis, drawing on Matrix row 1's requirement that all numerical evidence be attributable to a fixed, versioned, replayable input tuple to justify why a non-replayable feed cannot qualify either. This is flagged as synthesis rather than presented as a one-to-one citation, following the same convention §10 below uses.
...
3. No implementation decision may silently override a frozen specification. Post-freeze changes require an explicit amendment/ADR and a version change. *(Reconciliation §§1 and 9.)* Reconciliation §1 and §9 state this principle at the level of documents — no prior chat, research note, public product, or "normative" document may override the frozen specification (§9's Freeze Rule) — rather than at the level of implementation decisions made during or after build. Applying the same non-override principle to implementation decisions is an explicit synthesis, generalizing §9's document-level Freeze Rule to the build-time decisions that would otherwise be the most concrete way to silently defeat it. This is flagged as synthesis rather than presented as a one-to-one citation, following the same convention §10 below uses.
...
This Constitution may be revised before v1.0 freeze as contradictions are discovered during Architecture, Data/StrategyIR, Validation, Portfolio/Deployment, and Agent specification work. After v1.0 freeze, amendment requires an explicit ADR and version change — this document does not silently drift. *(Reconciliation §9.)* Reconciliation §9 (the Freeze Rule) states only that no prior document may override the frozen specification and that independent review precedes freeze; it does not itself spell out a pre-freeze revision process or a post-freeze ADR-and-version-change mechanic for this Constitution specifically. Both halves of this clause are an explicit synthesis of §9's Freeze Rule together with §0's Phase Rule (no implementation begins until the canonical specification set is completed, independently reviewed, and frozen), generalized into concrete pre-freeze/post-freeze amendment mechanics for this document. This is flagged as synthesis rather than presented as a one-to-one citation, following the same convention §10 above uses.
