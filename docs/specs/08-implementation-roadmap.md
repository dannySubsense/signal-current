...
contribution thresholds, the incubation-stage durations, and the agent-activation readiness checklist/
permission matrix). §3 is the sequencing table for all of these; §8 restates only this document's own four
new items (U-08/U-09/U-10/U-13) in the standard PROVISIONAL format.
...
set freezes. It is not a roadmap for finishing this spec-drafting sprint itself; that sprint's own
completion is tracked in `docs/specs/signal-current-v1-spec/PROGRESS.md` and its own North Star, which are
separate documents with a separate audience and separate success criteria.
...
|---|---|---|---|
| R1 | U-01a, U-01b, U-01c, U-02a, U-02b, U-02c, U-03 | None (earliest) | Dedicated schema/execution-semantics design sessions and ADR (Data Architecture §8) |
...
(Constitution §11), and MUST be revised once any Phase R1-R4 item, U-08, U-09, U-10, or U-13 is actually
resolved — at that point this document is amended (not silently superseded) to replace the relevant
PROVISIONAL tag or sequencing entry with a cited, versioned decision. After freeze, amendment requires an
explicit ADR and version change.
