...
| 59 | Independent reproduction requirement | `docs/NORTHSTAR.md` Thesis and Drift Check (2026-09-05); surfaced by Frank's spec-gate attempt 1 (2026-09-05) finding this requirement absent from the Matrix despite being the project's own declared falsifiable claim | Deterministic replay (row 22, §7.1) and multiple-testing diagnostics (row 33, §6 item 7) both constrain *the method* of validation; neither constrains *who* checks it | The Matrix and all eight canonical documents were drafted before this requirement existed anywhere in writing — every document faithfully traced a well that omitted the project's central claim | **A strategy is not validated by its own author's rerun, however deterministic the replay. Promotion to StrategyArtifact requires that someone other than the original author (or authoring agent) independently reruns the validation against the same content-addressed inputs and obtains the same result.** This is additional to, not a substitute for, deterministic replay and multiple-testing diagnostics. | No — principle is settled; ValidationPlan's exact reproduction-record schema and any numeric tolerance for "same result" remain open, tracked as new PROVISIONAL items, owner Danny |
...
14. A strategy is not validated by its own author's rerun, however deterministic the replay. Promotion to StrategyArtifact requires that someone other than the original author (or authoring agent) independently reruns the validation against the same content-addressed inputs and obtains the same result. (Matrix row 59; traces directly to `docs/NORTHSTAR.md` Thesis and Drift Check.)
